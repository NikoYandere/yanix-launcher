...
def open_winetricks():
    try:
        ...
        messagebox.showerror("Error", "Winetricks is not installed")
...
def open_settings():
    ...
    tk.Button(settings_window, text=LANGUAGES[current_lang]["winetricks"], command=open_winetricks).pack(pady=10)


    settings_window.protocol("WM_DELETE_WINDOW", lambda: on_close(settings_window))
