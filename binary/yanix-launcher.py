...
# Lista de diretórios onde pode estar o Yanix-Launcher
SEARCH_DIRS = [
    os.path.expanduser("~"),
...
if not os.path.exists(YANIX_PATH):
    ...
    buttons = [
        ...
        tk.Button(top, text=LANGUAGES[current_lang]["discord"], font=("Arial", 10, "bold"), command=open_discord),
        tk.Button(top, text=LANGUAGES[current_lang]["settings"], font=("Arial", 10, "bold"), command=open_settings)
    ]
    
    
    # Adicionando o botão de Exit
    exit_button = tk.Button(top, text=LANGUAGES[current_lang]["exit"], font=("Arial", 10, "bold"), command=exit_launcher)

    # Adicionando a versão no rodapé
    version_label = tk.Label(top, text=f"Version: {load_version()}", font=("Arial", 8), bg="white")
